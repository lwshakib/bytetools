...
export default function WeightConverterPage() {
  ...
  const sourceUnit =
    WEIGHT_UNITS.find((u) => u.code === fromUnit) || WEIGHT_UNITS[0];
  const targetUnit =
    WEIGHT_UNITS.find((u) => u.code === toUnit) || WEIGHT_UNITS[0];
        ...
        <div className="max-w-5xl mx-auto w-full flex flex-col items-center">
          ...
          <div className="flex flex-col items-center text-center space-y-4 mb-12 md:mb-20 w-full">
            ...
            <p className="text-muted-foreground text-[10px] font-medium uppercase tracking-[0.2em] opacity-40">
              High-Precision Mass Translation
            </p>
          </div>
                ...
                <div className="flex flex-wrap gap-10">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-bold tabular-nums">
                      1 {sourceUnit.code} ={' '}
                      {(
                        parseFloat(convertedAmount) / (parseFloat(amount) || 1)
                      ).toFixed(4)}{' '}
                      {targetUnit.code}
                    </p>
                  </div>
          ...
          <div className="w-full grid grid-cols-2 md:grid-cols-4 gap-4 mb-24">
            {[
              ...
              <div
                key={i}
                className="p-6 bg-card/40 border border-border/50 rounded-xl space-y-2"
              >
