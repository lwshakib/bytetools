        ...
        <div className="flex items-center justify-between p-4 border-b border-border/50 bg-background/80 backdrop-blur-md shrink-0 z-20">
          <div className="flex items-center gap-2 sm:gap-6">
            <TabsList className="bg-muted/30 p-1 h-9 gap-1 rounded-lg">
              <TabsTrigger
                value="daily"
                className="px-2 sm:px-4 rounded-md text-[10px] sm:text-xs font-medium transition-all"
              >
                Planner
              </TabsTrigger>
              <TabsTrigger
          ...
          {activeTab === 'daily' && (
            <div className="flex items-center gap-1 sm:gap-2">
              <Button
                variant="ghost"
                size="sm"
                className="hidden sm:inline-flex h-8 px-3 text-xs font-medium rounded-md hover:bg-muted"
              ...
              <div className="flex items-center rounded-md overflow-hidden border border-border/50">
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8 border-r border-border/50 rounded-none"
            ...
            <div className="flex h-full p-2 sm:p-6 gap-4 sm:gap-6 overflow-x-auto overflow-y-hidden md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 whitespace-nowrap md:whitespace-normal">
              {isLoading
                ? Array.from({ length: columnCount }).map((_, i) => (
                    <div
                      key={i}
                      className="flex flex-col h-full rounded-xl bg-card/40 border border-border/50 overflow-hidden min-w-[300px] md:min-w-0 transition-all shadow-sm"
                    >
                ...
                : currentColumns.map((date) => (
                    <DayColumn
                      ...
                      onAddTask={(text) =>
                        addTask(text, format(date, 'yyyy-MM-dd'), 'daily')
                      }
          ...
          <TabsContent value="dump" className="m-0 h-full overflow-y-auto">
            <TaskDumpTab
              ...
              onAddTask={(text: string) =>
                addTask(text, format(startOfToday(), 'yyyy-MM-dd'), 'dump')
              }
              onToggleTask={toggleTask}
              onDeleteTask={deleteTask}
              session={session}
...
function DayColumn({
  ...
  const isRoutineActiveOnDate = (routine: Routine, d: Date) => {
    ...
    if (routine.frequency === 'weekly')
      return routine.selectedDays.includes(getDay(d));
    ...
    if (routine.frequency === 'monthly')
      return routine.selectedDate === getDate(d);
    return false;
  };

  const tasksForThisDay = tasks.filter(
    (t) => t.date === dateKey && t.category === 'daily'
  );
  ...
  return (
    <div
      className={cn(
        'flex flex-col h-full rounded-xl bg-card/40 border border-border/50 overflow-hidden min-w-[300px] md:min-w-0 transition-all shadow-sm',
    ...
    >
        ...
        {isToday && pendingTasks.length > 0 && (
          <div className="space-y-2">
            <button
              ...
              className="w-full h-8 px-3 rounded-lg bg-amber-500/5 hover:bg-amber-500/10 text-[10px] font-bold uppercase tracking-wide flex items-center justify-between text-amber-600 transition-all"
            >
      ...
      <div className="p-4 border-t border-border/5 relative">
        {!session?.user && (
          <div
            className="absolute inset-0 z-20 flex items-center justify-center bg-background/40 backdrop-blur-[1px] cursor-pointer"
        ...
        <div className="flex items-center gap-2 bg-muted/20 border border-border/50 rounded-xl px-3 h-10">
          ...
          <input
            ...
            onKeyDown={(e) =>
              e.key === 'Enter' &&
              newTaskText.trim() &&
              (onAddTask(newTaskText), setNewTaskText(''))
            }
...
function TaskItem({
  ...
  return (
    <div
      className={cn(
        'group flex items-center gap-3 p-2.5 rounded-xl hover:bg-muted/30 transition-all',
    ...
    >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {task.routineId && !task.completed && (
          <span className="text-[8px] font-bold uppercase text-primary/40 tracking-wider mt-0.5 flex items-center gap-1">
            <Repeat className="w-2 h-2" /> Routine
          </span>
        )}
      </div>
      <Button
...
function RoutineTab({
  ...
  const handleCommit = () => {
    ...
    onAddRoutine(
      ...
      newFreq,
      selectedDays,
      newFreq === 'monthly' ? selectedDate : null
    );
    setNewText('');
    setSelectedDays([]);
  };
    ...
    <div className="max-w-2xl mx-auto p-8 space-y-8 pb-32">
      <div className="space-y-6">
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              className="h-10 bg-muted/20 border-border/50 rounded-lg px-3 text-sm font-medium"
            />
          </div>
          <div className="space-y-2">
            <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50">
              ...
              {(['daily', 'weekly', 'bi-weekly', 'monthly'] as const).map(
                (f) => (
                  <Button
                    ...
                    className={cn(
                      'flex-1 h-9 rounded-md text-[10px] font-bold uppercase transition-all',
                      newFreq === f
              ...
              <div className="flex flex-wrap gap-1">
                {WEEKDAYS.map((day, i) => (
                  <Button
                    ...
                    variant="ghost"
                    className={cn(
                      'h-9 w-9 rounded-md border text-[10px] font-bold uppercase transition-all',
              ...
              <div className="flex items-center gap-4">
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className="h-9 px-3 bg-muted/10 border-border/50 rounded-md font-bold text-xs gap-2"
                    >
                  ...
                  <PopoverContent
                    className="w-auto p-0 rounded-lg border-border shadow-xl"
                    align="start"
                  >
                    <Calendar
                      mode="single"
        ...
        <Button
          ...
          className="h-9 px-4 bg-primary text-primary-foreground hover:opacity-90 rounded-md font-bold uppercase tracking-widest text-[10px] w-full transition-all active:scale-95"
        >
          Add Routine
        </Button>
      </div>
          ...
          >
            <div className="flex items-center gap-3">
              <div className="px-2 py-0.5 bg-primary/5 border border-primary/10 rounded-full">
                <span className="text-[8px] font-bold uppercase tracking-tight text-primary/60">
                  {r.frequency}
                </span>
              </div>
            ...
            <Button
              variant="ghost"
              size="icon"
              className="h-7 w-7 opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive rounded-md transition-all"
      ...
      {completedTasks.length > 0 && (
        <div className="flex flex-col gap-6">
          <Button
            ...
            className="self-center h-10 px-6 rounded-lg text-[10px] font-bold uppercase tracking-widest opacity-40 hover:opacity-100 hover:bg-muted transition-all"
          >
            {showCompleted
              ? 'Hide Completed'
