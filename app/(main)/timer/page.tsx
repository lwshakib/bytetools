...
export default function TimerPage() {
  ...
  const handleStart = () => {
    const total =
      ...
      (parseInt(seconds) || 0);
    if (total === 0) {
      toast.error('Set duration first.');
      return;
    }
  ...
  const addPreset = () => {
    ...
    const total =
      (parseInt(newPresetH) || 0) * 3600 +
      (parseInt(newPresetM) || 0) * 60 +
      (parseInt(newPresetS) || 0);
    if (total === 0) {
      toast.error('Set time first');
      return;
    }
          ...
          <div className="max-w-2xl mx-auto w-full flex flex-col items-center">
            ...
            <div className="flex flex-col items-center text-center space-y-4 mb-12 sm:mb-20">
              ...
              <p className="text-muted-foreground text-[10px] font-medium uppercase tracking-[0.2em] opacity-40">
                Precision Countdown Synchronization
              </p>
            </div>
            ...
            <div className="relative mb-16 sm:mb-24 w-full">
              ...
              >
                <div className="flex justify-center items-center gap-2 md:gap-4 w-full">
                  ...
                  <div className="flex flex-col items-center gap-3">
                    <div className="flex bg-muted/20 border border-border/50 rounded-xl w-24 h-24 md:w-44 md:h-44 items-center justify-center relative">
                      {isEditing ? (
                        ...
                        <span className="text-5xl md:text-8xl font-bold tabular-nums tracking-tighter text-foreground">
                          {hours}
                        </span>
                      )}
                    </div>
                  ...
                  <span className="text-2xl md:text-5xl font-thin text-muted-foreground/20 self-center pb-8">
                    :
                  </span>
                  ...
                  <div className="flex flex-col items-center gap-3">
                    <div className="flex bg-primary/5 border border-primary/20 rounded-xl w-24 h-24 md:w-44 md:h-44 items-center justify-center relative">
                      {isEditing ? (
                        ...
                        <span className="text-5xl md:text-8xl font-bold tabular-nums tracking-tighter text-primary">
                          {seconds}
                        </span>
                      )}
                    </div>
        ...
        <div className="w-full lg:w-[300px] border-l border-border/50 bg-muted/5 flex flex-col shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-10">
            <div className="space-y-4">
              <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/20 px-1">
                Custom
              </span>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <p className="text-[10px] text-muted-foreground uppercase font-medium">
              Save the current temporal cycle as a preset profile.
            </p>
          </DialogHeader>
          <div className="py-6 space-y-6">
            ...
            <div className="space-y-4">
              <Label className="text-[9px] font-bold uppercase tracking-widest px-1 text-muted-foreground/50">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2">
                  <Input
                    value={newPresetH}
                    onChange={(e) =>
                      handleInputChange(e.target.value, setNewPresetH, 99)
                    }
                  ...
                  />
                  <p className="text-center text-[9px] uppercase font-bold text-muted-foreground/30 tracking-tight">
                    Hours
                  </p>
                </div>
                <div className="space-y-2">
                  <Input
                    value={newPresetM}
                    onChange={(e) =>
                      handleInputChange(e.target.value, setNewPresetM, 59)
                    }
                    className="h-14 text-center text-2xl font-bold bg-muted/20 border-border/50 focus:border-primary/50 focus:ring-1 focus:ring-primary/20 transition-all tabular-nums rounded-xl"
                  />
                  <p className="text-center text-[9px] uppercase font-bold text-muted-foreground/30 tracking-tight">
                ...
                <div className="space-y-2">
                  <Input
                    value={newPresetS}
                    onChange={(e) =>
                      handleInputChange(e.target.value, setNewPresetS, 59)
                    }
                  ...
                  />
                  <p className="text-center text-[9px] uppercase font-bold text-primary/40 tracking-tight">
                    Seconds
                  </p>
                </div>
              </div>
