            ...
            <div className="md:col-span-8 space-y-8">
                    ...
                    <CardContent className="p-6">
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50">Secret Key</Label>
                                <Button 
                                    variant="ghost" 
                                    size="sm" 
                ...
                <Tabs defaultValue="encode" className="w-full">
                    <TabsList className="grid w-full grid-cols-2 h-10 bg-muted/40 p-1 rounded-lg border border-border/50">
                        <TabsTrigger value="encode" className="rounded-md text-[10px] font-bold uppercase transition-all">Encode</TabsTrigger>
                        <TabsTrigger value="decode" className="rounded-md text-[10px] font-bold uppercase transition-all">Decode</TabsTrigger>
                    </TabsList>
                        ...
                        <div className="grid grid-cols-1 gap-6">
                            <Card className="bg-card/40 border-border/50 rounded-2xl overflow-hidden">
                                <CardHeader className="p-6 pb-2">
                                    <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50">Payload JSON</CardTitle>
                                </CardHeader>
                            ...
                            {encodedToken && (
                                <Card className="bg-card/40 border-border/50 rounded-2xl overflow-hidden">
                                    <CardHeader className="p-6 pb-2">
                                        <div className="flex items-center justify-between">
                                            <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50">Encoded Result</CardTitle>
                                            <Button
                                                variant="ghost"
                                                size="sm"
                                            ...
                                            >
                                                {copiedType === 'Token' ? <Check className="w-3 h-3 mr-2" /> : <Copy className="w-3 h-3 mr-2" />}
                                                Copy
                                            </Button>
                                        </div>
                                    </CardHeader>
                    ...
                    <TabsContent value="decode" className="mt-6 space-y-6">
                        <Card className="bg-card/40 border-border/50 rounded-2xl overflow-hidden">
                            <CardHeader className="p-6 pb-2">
                                <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/50">Token to decode</CardTitle>
                            </CardHeader>
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                <Card className="bg-card/40 border-border/50 rounded-2xl overflow-hidden">
                                    <CardHeader className="p-6 pb-2 border-b border-border/5 flex flex-row items-center justify-between">
                                        <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-primary/60">Header</CardTitle>
                                        <Button variant="ghost" size="icon" className="h-6 w-6 rounded-md" onClick={() => copyToClipboard(JSON.stringify(decodedHeader, null, 2), 'Header')}>
                                ...
                                <Card className="bg-card/40 border-border/50 rounded-2xl overflow-hidden">
                                    <CardHeader className="p-6 pb-2 border-b border-border/5 flex flex-row items-center justify-between">
                                        <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-primary/60">Payload</CardTitle>
                                        <Button variant="ghost" size="icon" className="h-6 w-6 rounded-md" onClick={() => copyToClipboard(JSON.stringify(decodedPayload, null, 2), 'Payload')}>
                                            <Copy className="w-3 h-3" />
                                        </Button>
                                    </CardHeader>
                                    <CardContent className="p-6">
                                ...
                                <div className={cn(
                                    ...
                                    isVerified === true ? "bg-emerald-500/5 border-emerald-500/20 text-emerald-600" :
                                    isVerified === false ? "bg-red-500/5 border-red-500/20 text-red-600" :
                                    "bg-muted/10 border-border/50 text-muted-foreground"
                                )}>
                                    <div className="flex items-center gap-3">
                                        {isVerified === true ? <ShieldCheck className="w-5 h-5" /> : isVerified === false ? <ShieldAlert className="w-5 h-5" /> : <Info className="w-5 h-5" />}
                                        <span className="text-[10px] font-bold uppercase tracking-widest">
                                            {isVerified === true ? 'Signature Verified' : isVerified === false ? 'Invalid Signature' : 'No Secret Provided'}
                                        </span>
                                    </div>
                                    <div className="flex gap-4">
                                        <div className="flex flex-col items-end">
                                            <span className="text-[8px] font-bold uppercase opacity-40">Algorithm</span>
                                            <span className="text-xs font-mono font-bold">{decodedHeader?.alg || '---'}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div className="md:col-span-4 space-y-8">
                {session?.user ? (
                    <Card className="bg-card/40 border-border/50 shadow-sm rounded-2xl overflow-hidden h-fit">
                        <CardHeader className="p-6 pb-2">
                            <CardTitle className="text-sm font-bold uppercase tracking-widest text-muted-foreground/50">Vault</CardTitle>
                        </CardHeader>
                        <CardContent className="p-6 pt-0">
                            {savedJwts.length === 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-8 bg-primary/5 border border-primary/10 rounded-2xl cursor-pointer text-center space-y-3" onClick={() => document.getElementById('signin-trigger')?.click()}>
                        <Lock className="w-6 h-6 text-primary/60 mx-auto" />
                        <span className="text-[10px] font-bold uppercase tracking-widest text-primary/60">Sign in to sync tokens</span>
                    </div>
                )}
            </div>
        </div>
