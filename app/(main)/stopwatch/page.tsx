...
export default function StopwatchPage() {
  ...
  const averageLap =
    laps.length > 0 ? laps.reduce((a, b) => a + b, 0) / laps.length : 0;
      ...
      <div className="w-full h-full max-w-5xl mx-auto flex flex-col items-center">
        ...
        {!isFullscreen && (
          <div className="flex flex-col items-center text-center space-y-4 mb-12 sm:mb-16">
            ...
            <p className="text-muted-foreground text-[10px] font-medium uppercase tracking-[0.2em] opacity-40">
              Precision Millisecond Synchronization
            </p>
          </div>
        )}
        ...
        >
          ...
          >
            <div className="flex items-baseline gap-1">
              <span className="text-[72px] sm:text-[100px] md:text-[140px] font-bold tracking-tighter tabular-nums leading-none">
                {t.m}
                <span className="text-muted-foreground/10 mx-1">:</span>
                {t.s}
              </span>
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-4 mt-8 sm:mt-16 pb-1 flex-wrap">
            ...
            >
              {isRunning ? (
                <Pause className="w-3.5 h-3.5 sm:w-4 h-4 fill-current" />
              ) : (
                <Play className="w-3.5 h-3.5 sm:w-4 h-4 fill-current" />
              )}
            ...
            >
              ...
              <div className="lg:col-span-4 space-y-3">
                ...
                <div className="space-y-2">
                  {[
                    {
                      label: 'Delta Average',
                      val: formatTime(averageLap),
                      icon: Activity,
                      color: 'text-primary',
                    },
                    {
                      label: 'Fastest Mark',
                      val: formatTime(laps[bestLapIndex] || 0),
                      icon: TrendingUp,
                      color: 'text-emerald-500',
                    },
                    {
                      label: 'Slowest Mark',
                      val: formatTime(laps[worstLapIndex] || 0),
                      icon: TrendingDown,
              ...
              <div className="lg:col-span-8 flex flex-col space-y-3">
                <div className="flex items-center justify-between px-1">
                  ...
                  <span className="text-[8px] font-bold uppercase tracking-widest text-muted-foreground/20 italic">
                    Last Updated: {t.m}:{t.s}
                  </span>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-6">
                          <span className="text-[9px] font-bold text-muted-foreground/20 tabular-nums">
                            #{idx.toString().padStart(2, '0')}
                          </span>
                          <div className="flex flex-col">
                            <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/40 leading-none mb-1.5">
                              Mark Capture
                            </span>
                            <div className="flex items-baseline gap-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {i === worstLapIndex && (
                            <TrendingDown className="w-4 h-4 text-amber-500/50" />
                          )}
