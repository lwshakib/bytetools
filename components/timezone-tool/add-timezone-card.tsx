  ...
  return (
    <Card
      ...
      className={cn(
        ...
        !isSearching &&
          session?.user &&
          'cursor-pointer hover:bg-accent/50 hover:border-accent border-dashed flex flex-col items-center justify-center text-center',
        !session?.user &&
          'border-dashed flex flex-col items-center justify-center text-center'
      )}
      onClick={() =>
        isSearching || !session?.user ? null : setIsSearching(true)
      }
    >
          ...
          <div className="flex flex-col items-center gap-4 px-6 text-center transition-transform group-hover/auth:scale-105">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[9px] font-medium text-muted-foreground uppercase tracking-widest leading-relaxed">
                Auth required to add custom timezones to your global grid.
              </p>
            </div>
          </div>
        </div>
