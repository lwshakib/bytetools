    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {date ? format(date, "PPP " + (use12Hour ? "p" : "HH:mm")) : <span>{label || "Pick date & time"}</span>}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-auto p-0 flex flex-col md:flex-row rounded-xl border-border shadow-2xl bg-card/95 backdrop-blur-xl shrink-0" align="start">
          ...
          <div className="flex flex-col border-t md:border-t-0 md:w-36 bg-muted/5">
            <div className="p-3 border-b border-border/50 bg-muted/10 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/40">Configuration</span>
                <button 
                ...
                <div className="flex rounded-md border border-border/50 overflow-hidden">
                  <button
                    ...
                    className={cn(
                      ...
                      selectedDate.getHours() < 12 ? "bg-primary text-primary-foreground" : "text-muted-foreground/50 hover:text-foreground"
                    )}
                  >
                    AM
                  </button>
